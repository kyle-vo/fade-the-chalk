    ...
    for g in games:
        ...
        for side, opp in (('home', 'away'), ('away', 'home')):
            ...
            for slot, pl in enumerate(lineup[:9]):
                ...
                rows.append({'sport': 'MLB', 'id': pid, 'name': pl.get('fullName') or per.get('fullName'), 'team': team['abbreviation'] if 'abbreviation' in team else team['name'],
...
def nfl():
    ...
    for e in sb['events']:
        ...
        games.append({'name': e['shortName'], 'date': e['date'], 'state': c['status']['type']['name'], 'home': hs, 'away': aw, 'total': total or None, 'spread': o.get('details'),
    ...
    for g in games:
        for team in (g['home'], g['away']):
            ...
            for r, p, share, inj in cands:
                ...
                rows.append({'sport': 'NFL', 'id': r['id'], 'name': r['name'], 'team': team, 'opp': opp, 'pos': r['pos'], 'game': g['name'], 'state': g['state'], 'time': g['date'],
