...
for lockf in sorted(glob.glob(os.path.join(BT, 'ml_*.json'))):
    ...
    for r in rows:
        ...
        if r['homeWin'] is not None and r.get('pubHome') is not None and r.get('kvol'):
            th, ta = r.get('takerHome$') or 0, r.get('takerAway$') or 0
            if th + ta:                                                       # trade tape: dollars the crowd actually put on each team (same source as Public $ on pick)
                lose, win = (ta, th) if r['homeWin'] == 1 else (th, ta)
                r['bookTake'] = round(lose); r['bookGave'] = round(win); r['loserShare'] = round(lose / (th + ta), 3); r['takeSrc'] = 'tape'; r['takePool'] = round(th + ta)
            else:                                                             # no tape: fall back to the volume split (understates money on big favorites)
                loser_share = (1 - r['pubHome']) if r['homeWin'] == 1 else r['pubHome']
                r['bookTake'] = round(loser_share * r['kvol']); r['loserShare'] = round(loser_share, 3); r['bookGave'] = round((1 - loser_share) * r['kvol']); r['takeSrc'] = 'volume'; r['takePool'] = r['kvol']
...
function verdict(r){
  ...
  // MLB: edge = model % minus Robinhood's ask. Through 09-17 on 64 graded MLB games the profit is all in big edges:
...
function render(){
  ...
  const cols = [['Game', 'time'], ['Pick', 'model'], ['Model', 'model'], ['Pinnacle', 'sharp'], ['Diff', 'diff'], ['Robinhood', 'edge'], ['Edge', 'edge'], ['Public $ on pick', 'pub'], ['$ at risk', 'kvol'], ['Verdict', 'v'], ['Bet', 'v'], ['Result', 'v'], ['Book take', 'take'], ['Book gave', 'gave']];   // tape columns stay in the data (takerHome$/takerAway$) and in the scorecard, just not on the board
  const R = new Set(['Model', 'Pinnacle', 'Diff', 'Robinhood', 'Edge', 'Public $ on pick', '$ at risk', 'Book take', 'Book gave']);
  ...
  for (const x of list) { const r = x.r, e = store[key(r)] || {};
    ...
    tr.innerHTML = `<td><span class="nm">${r.away} @ ${r.home}</span>${r.mostBet ? ' <span class="tm" title="one of the slate's most-bet games by $ at risk. A flag, not a verdict: once the model-backed dogs are set aside, the dog has NOT beaten its price here" style="color:var(--warn)">🔥 most-bet</span>' : ''}<br><span class="tm">${when}${r.homeSP ? ' · ' + r.awaySP + ' / ' + r.homeSP : ''}${r.homeRec ? ' · ' + r.awayRec + ' / ' + r.homeRec : ''}</span></td>
      ...
      <td class="num" title="${r.bookGave != null ? 'winning side had ' + Math.round((1 - r.loserShare) * 100) + '% of the $' + (r.takePool || r.kvol).toLocaleString() + (r.takeSrc === 'tape' ? ' the crowd put on this game (trade tape)' : ' traded (volume split, no tape)') : 'fills in when the game is final'}">${r.bookGave != null ? '$' + (r.bookGave >= 1000 ? Math.round(r.bookGave / 1000) + 'k' : r.bookGave) : '—'}</td>`;
