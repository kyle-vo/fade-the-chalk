...
function renderSlate(list){
  ...
  const byDay = {};
  for (const x of list) { const d = new Date(x.r.time); const k = d.toLocaleDateString([], { weekday: 'long', month: 'short', day: 'numeric' }); (byDay[k] = byDay[k] || { t: d.getTime(), items: [] }).items.push(x); }
  ...
  $('#slate').innerHTML = days.map(([label, d]) => `<div class="sday">${label}</div><div class="sgrid">` + d.items.sort((a, b) => new Date(a.r.time) - new Date(b.r.time)).map(x => {
    ...
    const side = (ab, isHome) => { const pk = (isHome === pickHome); const p = isHome ? r.model : 1 - r.model; const odds = isHome ? r.rhHome : r.rhAway;
      return `<div class="side ${pk ? 'pick' : ''} ${winner === (isHome ? 'home' : 'away') ? 'winner' : ''}"><img src="${logo(sp, ab)}" alt="${ab}" loading="lazy"><span class="ab">${ab}</span><span class="pr">${pct(p)}${odds != null ? ' · ' + fmt(odds) : ''}</span></div>`; };
    const when = new Date(r.time).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' });
    return `<div class="mcard ${cls} ${done ? (r.pickHit ? 'won' : 'lost') : ''}" title="${r.away} @ ${r.home}${r.homeSP ? ' · ' + r.awaySP + ' / ' + r.homeSP : ''}">${side(r.away, false)}<div class="mid"><span class="at">AT</span><span class="tm">${when}</span>${x.pub != null ? `<span title="public $ on the pick">${pct(x.pub)} pub</span>` : ''}</div>${side(r.home, true)}
      <div class="foot"><span class="tag ${cls}">${x.v}${r.edge != null ? ' · ' + (r.edge >= 0 ? '+' : '') + r.edge.toFixed(1) : ''}</span><span>${done ? `<span class="res ${r.pickHit ? 'win' : 'loss'}">${r.pickHit ? '✓ ' : '✗ '}${winner === 'home' ? r.home : r.away} ${r.score || ''}</span>` : (r.homeSP ? r.awaySP + ' / ' + r.homeSP : '')}</span></div></div>`; }).join('') + '</div>').join('') || '<div class="empty">Nothing to show with these filters.</div>';
}
function render(){
...
<style>.vsw{{margin-left:auto;display:inline-flex;border:1px solid var(--line);border-radius:6px;overflow:hidden}}.vb{{background:var(--panel);color:var(--mute);border:0;padding:5px 12px;cursor:pointer;font:inherit}}.vb.on{{background:#1b2129;color:var(--ink)}}.sday{{font-family:var(--disp);font-size:20px;letter-spacing:1px;text-transform:uppercase;color:var(--acc);margin:16px 0 8px}}.sday:first-child{{margin-top:4px}}.sgrid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:10px}}.mcard{{background:#0e1115;border:1px solid var(--line);border-radius:10px;padding:10px 12px;display:grid;grid-template-columns:1fr auto 1fr;grid-template-rows:auto auto;column-gap:8px;align-items:center;position:relative}}.mcard .side{{display:flex;flex-direction:column;align-items:center;gap:4px;padding:8px 6px;border-radius:8px;border:2px solid transparent;opacity:.45;transition:opacity .15s}}.mcard .side img{{width:64px;height:64px;object-fit:contain;filter:drop-shadow(0 2px 6px rgba(0,0,0,.6))}}.mcard .side .ab{{font-family:var(--disp);font-size:20px;letter-spacing:1px}}.mcard .side .pr{{font:12px var(--mono);color:var(--mute)}}.mcard .side.pick{{opacity:1;border-color:var(--acc);background:#1a1607;box-shadow:0 0 0 1px #3a2f0a,0 0 18px rgba(255,176,32,.25)}}.mcard .side.pick .pr{{color:var(--acc)}}.mcard.strong .side.pick{{border-color:var(--good);background:#0f2117;box-shadow:0 0 0 1px #14402a,0 0 18px rgba(47,212,122,.25)}}.mcard.strong .side.pick .pr{{color:var(--good)}}.mcard.pass .side.pick{{border-color:#3a4450;background:#151a21;box-shadow:none}}.mcard.pass .side.pick .pr{{color:var(--mute)}}.mcard .mid{{display:flex;flex-direction:column;align-items:center;gap:3px;font:12px var(--mono);color:var(--mute);min-width:64px}}.mcard .mid .at{{font-family:var(--disp);font-size:14px;letter-spacing:1px}}.mcard .mid .tm{{font-size:11px;white-space:nowrap}}.mcard .foot{{grid-column:1/4;display:flex;justify-content:space-between;align-items:center;margin-top:6px;font-size:11px;color:var(--mute)}}.mcard .tag{{font:600 10px var(--mono);letter-spacing:.6px;text-transform:uppercase;padding:2px 7px;border-radius:5px;border:1px solid var(--line)}}.mcard .tag.strong{{color:var(--good);border-color:#14402a}}.mcard .tag.bet{{color:var(--blue);border-color:#173a5c}}.mcard .tag.pass{{color:var(--mute)}}.mcard .res{{font-weight:700}}.mcard .res.win{{color:var(--good)}}.mcard .res.loss{{color:var(--bad)}}.mcard.won{{border-color:#14402a}}.mcard.lost{{border-color:#3a2a2e}}.mcard.lost .side.pick{{opacity:.6}}.mcard .winner .ab{{text-decoration:underline;text-decoration-color:var(--good);text-underline-offset:4px}}</style>
